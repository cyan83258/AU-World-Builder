    ...
    A.bindUIEvents = function () {
        ...
        (function () {
            ...
            bindClick('auwb-combo-roll', function () {
                var s = A.getSettings();
                document.getElementById('auwb-combo-setting').textContent  = _rand(A.COMBO_SETTINGS.concat(s.userComboSettings || []));
                document.getElementById('auwb-combo-relation').textContent = _rand(A.COMBO_RELATIONS.concat(s.userComboRelations || []));
                document.getElementById('auwb-combo-twist').textContent    = _rand(A.COMBO_TWISTS.concat(s.userComboTwists || []));
            });
            ...
            var _slotMap = {
                settings:  { builtIn: A.COMBO_SETTINGS,  userKey: 'userComboSettings',  label: '배경' },
                relations: { builtIn: A.COMBO_RELATIONS, userKey: 'userComboRelations', label: '관계' },
                twists:    { builtIn: A.COMBO_TWISTS,    userKey: 'userComboTwists',    label: '반전 요소' },
            };
            ...
            function _renderSlotEditor() {
                ...
                if (!info) return;
                var s = A.getSettings();
                var userArr = s[info.userKey] || [];
                var html = '';
                info.builtIn.forEach(function (it) {
                    html += '<div class="auwb-idea-item auwb-slot-item auwb-slot-builtin">'
                        + '<span class="auwb-idea-custom-text">' + A.escapeHtml(it) + '</span>'
                        + '<span class="auwb-slot-badge"><i class="fa-solid fa-lock"></i></span>'
                        + '</div>';
                });
                userArr.forEach(function (it, i) {

...
(function (A) {
    ...
    /* ══════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════ */
    var _secMgrDirty = true;
    A.renderSectionManager = function () {
        var container = document.getElementById('auwb-section-manager');
        if (!container) return;
        /* Skip if settings tab not visible — mark dirty for lazy render */
        var tabEl = document.getElementById('auwb-tab-settings');
        if (tabEl && tabEl.style.display === 'none') { _secMgrDirty = true; return; }
        _secMgrDirty = false;

        var s     = A.getSettings();
        var items = [];

        var directives = (s.genOptions || {}).sectionDirectives || {};

        A.getAllSections().forEach(function (sec) {
            items.push({
                id: sec.id, label: sec.label, isBuiltIn: sec.isBuiltIn,
                enabled: sec.enabled, locked: A.isSectionLocked(sec.id),
                injPos: sec.injPos, injDepth: sec.injDepth, injRole: sec.injRole,
                directive: directives[sec.id] || '',
            });
        });
        ...
        items.push({
            ...
            enabled:  gc.enabled !== false, locked: !!gc.locked,
        ...
        items.forEach(function (it) {
            ...
            html += '<div class="auwb-secmgr-item" data-sec-id="' + it.id + '"' + (isDraggable ? ' draggable="true"' : '') + '>'
                ...
                + '<button class="auwb-secmgr-lock auwb-preset-btn' + (it.locked ? ' active' : '') + '" title="' + (it.locked ? '잠금 해제' : '잠금 (업데이트 보호)') + '">'
                + '<i class="fa-solid ' + (it.locked ? 'fa-lock' : 'fa-lock-open') + '"></i></button>';
            ...
            html += '<button class="auwb-secmgr-expand auwb-preset-btn" title="주입 설정">'
                ...
                + '</select></div>'
                + '</div>'
                + '<div class="auwb-input-group" style="margin-top:6px;"><label>섹션 지시문 <small style="color:var(--auwb-text-muted);">(이 섹션에만 적용되는 커스텀 지시)</small></label>'
                + '<input type="text" class="auwb-input auwb-secmgr-directive" data-sec-id="' + it.id + '" value="' + A.escapeHtml(it.directive || '') + '" placeholder="예: 내면의 모순을 극단적으로 강조…" maxlength="200">'
                + '</div>'
                + '</div></div>';
        });
    ...
    A.updateSectionDisabledVisuals = function () {
        A.BUILTIN_SECTIONS.forEach(function (bs) {
            ...
            if (A.isSectionLocked(bs.id)) sec.classList.add('auwb-section-locked');
            else sec.classList.remove('auwb-section-locked');
        });
    };
    ...
    /* ══════════════════════════════════════════════
       Diff View (feature C)
       ══════════════════════════════════════════════ */
    var _diffSideBySide = false;
    var _lastDiffBefore = null, _lastDiffAfter = null;

    A.showDiffView = function (before, after) {
        _lastDiffBefore = before;
        _lastDiffAfter  = after;
        _renderDiffContent(before, after);
        var modal = document.getElementById('auwb-diff-modal');
        if (modal) modal.style.display = 'flex';
    };

    function _renderDiffContent(before, after) {
        ...
        diffs.forEach(function (d) {
            ...
            if (_diffSideBySide) {
                /* Side-by-side view */
                var removed = [], added = [], same = [];
                d.lines.forEach(function (l) {
                    if (l.type === 'removed')     removed.push(l.text);
                    else if (l.type === 'added')   added.push(l.text);
                    else { removed.push(l.text); added.push(l.text); }
                });
                html += '<div class="auwb-diff-side-by-side">'
                    + '<div class="auwb-diff-side auwb-diff-side-old"><div class="auwb-diff-side-label">이전</div>';
                removed.forEach(function (t) { html += '<div class="auwb-diff-removed">' + A.escapeHtml(t) + '</div>'; });
                html += '</div><div class="auwb-diff-side auwb-diff-side-new"><div class="auwb-diff-side-label">이후</div>';
                added.forEach(function (t) { html += '<div class="auwb-diff-added">' + A.escapeHtml(t) + '</div>'; });
                html += '</div></div>';
            } else {
                /* Unified diff view */
                html += '<div class="auwb-diff-lines">';
                d.lines.forEach(function (l) {
    ...
    function _initDelegation() {
        ...
        if (secMgr) {
            secMgr.addEventListener('change', function (e) {
                ...
                } else if (e.target.classList.contains('auwb-secmgr-directive')) {
                    var gs = A.getSettings();
                    if (!gs.genOptions) gs.genOptions = {};
                    if (!gs.genOptions.sectionDirectives) gs.genOptions.sectionDirectives = {};
                    gs.genOptions.sectionDirectives[sid] = e.target.value;
            ...
            secMgr.addEventListener('click', async function (e) {
                ...
                if (e.target.closest('.auwb-secmgr-lock')) {
                    var isLocked = A.isSectionLocked(sid);
                    A.updateSectionConfig(sid, 'locked', !isLocked);
                    A.renderSectionManager();
                    A.updateSectionDisabledVisuals();
                    A.showStatus(isLocked ? '잠금 해제됨' : '섹션 잠금됨 (업데이트 보호)', 'info');
                }
            });
    ...
    function initGenOptions() {
        var maps = [
            ...
            { prefix: 'auwb-opt-original',     key: 'original',    vals: ['break', 'loose', 'core', 'keep'] },
            { prefix: 'auwb-opt-mood',         key: 'mood',        vals: ['humorous', 'light', 'bittersweet', 'dark', 'grim'] },
        ...
        /* Advanced toggle switches (2-Pass, Sequential, Self-Critique) */
        initAdvancedToggles();

        /* Text inputs (theme anchor, dramatic question, must-include/exclude) */
        initCreativeDirection();

        /* Source detail fine-grained control */
        initSourceDetail();
    ...
    /* ══════════════════════════════════════════════
       Mood Presets (톤/무드 프리셋)
       ══════════════════════════════════════════════ */
    var BUILTIN_MOOD_PRESETS = [
        { name: '🌸 로맨틱 코미디',    opts: { mood: 'light',       conflict: 'subtle',  cliche: 'allow',   detailDepth: 'normal' }, tags: ['로맨스', '코미디'] },
        { name: '🖤 다크 앵스트',      opts: { mood: 'dark',        conflict: 'central', cliche: 'subvert', detailDepth: 'detailed' }, tags: ['앵스트', '느와르'] },
        { name: '🍂 씁쓸한 힐링',      opts: { mood: 'bittersweet', conflict: 'subtle',  cliche: 'allow',   detailDepth: 'normal' }, tags: ['힐링'] },
        { name: '⚔️ 긴장감 액션',     opts: { mood: 'dark',        conflict: 'central', cliche: 'subvert', detailDepth: 'detailed' }, tags: ['액션', '판타지'] },
        { name: '☕ 일상 힐링',        opts: { mood: 'light',       conflict: 'none',    cliche: 'allow',   detailDepth: 'normal' }, tags: ['일상', '힐링'] },
        { name: '🔮 미스터리 서스펜스', opts: { mood: 'bittersweet', conflict: 'central', cliche: 'subvert', detailDepth: 'detailed' }, tags: ['미스터리', '호러'] },
    ];

    function renderMoodPresets() {
        var container = document.getElementById('auwb-mood-preset-btns');
        if (!container) return;
        var s = A.getSettings();
        var userPresets = s.moodPresets || [];
        var all = BUILTIN_MOOD_PRESETS.concat(userPresets);

        var html = '';
        all.forEach(function (p, i) {
            var isUser = i >= BUILTIN_MOOD_PRESETS.length;
            html += '<div class="auwb-mood-preset-item" data-preset-idx="' + i + '">'
                + '<span class="auwb-mood-preset-name">' + A.escapeHtml(p.name) + '</span>';
            if (isUser) html += '<button class="auwb-mood-preset-del" data-preset-idx="' + i + '" title="삭제"><i class="fa-solid fa-xmark"></i></button>';
            html += '</div>';
        });
        container.innerHTML = html;
    }
    ...
    function applyMoodPreset(preset) {
        ...
        Object.keys(preset.opts).forEach(function (key) {
            s.genOptions[key] = preset.opts[key];
            /* Update toggle buttons in UI */
            var prefix = 'auwb-opt-' + key;
            document.querySelectorAll('[id^="' + prefix + '-"]').forEach(function (b) { b.classList.remove('active'); });
            var activeBtn = document.getElementById(prefix + '-' + preset.opts[key]);
            if (activeBtn) activeBtn.classList.add('active');
        });
        /* Apply genre tags */
        if (preset.tags && preset.tags.length) {
            s.genOptions.genreTags = preset.tags.slice();
            renderGenreTags();
        }
        /* Apply style tags */
        if (preset.styles && preset.styles.length) {
            s.genOptions.styleTags = preset.styles.slice();
            renderStyleTags();
        }
        /* Apply era tags */
        if (preset.eras && preset.eras.length) {
            s.genOptions.eraTags = preset.eras.slice();
            renderEraTags();
        }
        A.saveSetting('genOptions', s.genOptions);
        A.showStatus('프리셋 적용: ' + preset.name, 'success');
    }
    ...
    function initMoodPresets() {
        ...
        if (container) {
            container.addEventListener('click', function (e) {
                /* Delete user preset */
                var delBtn = e.target.closest('.auwb-mood-preset-del');
                if (delBtn) {
                    var idx = parseInt(delBtn.getAttribute('data-preset-idx'));
                    var userIdx = idx - BUILTIN_MOOD_PRESETS.length;
                    var s = A.getSettings();
                    var arr = s.moodPresets || [];
                    arr.splice(userIdx, 1);
                    A.saveSetting('moodPresets', arr);
                    renderMoodPresets();
                    A.showStatus('프리셋 삭제됨', 'info');
                    return;
                }
                /* Apply preset */
                var item = e.target.closest('.auwb-mood-preset-item');
                if (!item) return;
                var idx2 = parseInt(item.getAttribute('data-preset-idx'));
                var all = BUILTIN_MOOD_PRESETS.concat(A.getSettings().moodPresets || []);
                if (all[idx2]) applyMoodPreset(all[idx2]);
            });
        }

        /* Save current options as a user preset */
        bindClick('auwb-save-mood-preset', function () {
            var nameInput = document.getElementById('auwb-mood-preset-name');
            if (!nameInput) return;
            var name = nameInput.value.trim();
            if (!name) { A.showStatus('프리셋 이름을 입력하세요.', 'error'); return; }
            var s = A.getSettings();
            var opts = s.genOptions || {};
            var newPreset = {
                name: name,
                opts: {
                    mood:        opts.mood || 'light',
                    conflict:    opts.conflict || 'subtle',
                    cliche:      opts.cliche || 'allow',
                    detailDepth: opts.detailDepth || 'normal',
                    original:    opts.original || 'loose',
                    relation:    opts.relation || 'first',
                },
                tags: (opts.genreTags || []).slice(),
                styles: (opts.styleTags || []).slice(),
                eras: (opts.eraTags || []).slice(),
            };
            var arr = s.moodPresets || [];
            arr.push(newPreset);
            A.saveSetting('moodPresets', arr);
            nameInput.value = '';
            renderMoodPresets();
            A.showStatus('프리셋 저장됨: ' + name, 'success');
        });
    }
    ...
    /* ══════════════════════════════════════════════
       Advanced Generation Toggles (2-Pass, Sequential, Self-Critique)
       ══════════════════════════════════════════════ */
    function initAdvancedToggles() {
        var opts = A.getSettings().genOptions || {};

        var toggleMap = [
            { id: 'auwb-two-pass-toggle',      key: 'twoPassEnabled' },
            { id: 'auwb-sequential-toggle',     key: 'sequentialEnabled' },
            { id: 'auwb-self-critique-toggle',  key: 'selfCritiqueEnabled' },
        ];

        toggleMap.forEach(function (t) {
            var el = document.getElementById(t.id);
            if (!el) return;
            el.checked = !!opts[t.key];
            el.addEventListener('change', function () {
                var s = A.getSettings();
                if (!s.genOptions) s.genOptions = {};
                s.genOptions[t.key] = el.checked;
                /* Mutual exclusivity: sequential overrides 2-pass */
                if (t.key === 'sequentialEnabled' && el.checked) {
                    s.genOptions.twoPassEnabled = false;
                    var tp = document.getElementById('auwb-two-pass-toggle');
                    if (tp) tp.checked = false;
                } else if (t.key === 'twoPassEnabled' && el.checked) {
                    s.genOptions.sequentialEnabled = false;
                    var sq = document.getElementById('auwb-sequential-toggle');
    ...
    /* ══════════════════════════════════════════════
       Creative Direction (Theme Anchor, Dramatic Question, Must-Include/Exclude)
       ══════════════════════════════════════════════ */
    function initCreativeDirection() {
        var opts = A.getSettings().genOptions || {};

        var fields = [
            { id: 'auwb-theme-anchor',     key: 'themeAnchor' },
            { id: 'auwb-dramatic-question', key: 'dramaticQuestion' },
            { id: 'auwb-must-include',      key: 'mustInclude' },
            { id: 'auwb-must-exclude',      key: 'mustExclude' },
        ];
        ...
        fields.forEach(function (f) {
            ...
            el.addEventListener('change', function () {
                var s = A.getSettings();
                if (!s.genOptions) s.genOptions = {};
                s.genOptions[f.key] = el.value.trim();
                A.saveSetting('genOptions', s.genOptions);
            });
        });
    }
    ...
    /* ══════════════════════════════════════════════
       Source Detail Fine-Grained Control (원작 세부 반영 항목)
       ══════════════════════════════════════════════ */
    function initSourceDetail() {
        var opts = A.getSettings().genOptions || {};
        var toggle = document.getElementById('auwb-source-detail-toggle');
        var panel = document.getElementById('auwb-source-detail-panel');
        if (!toggle || !panel) return;

        /* Restore enabled state */
        toggle.checked = !!opts.sourceDetailEnabled;
        panel.style.display = toggle.checked ? '' : 'none';

        /* Restore aspect buttons */
        var aspects = opts.sourceDetailAspects || {};
        panel.querySelectorAll('button[data-aspect]').forEach(function (btn) {
            var key = btn.getAttribute('data-aspect');
            if (aspects[key]) btn.classList.add('active');
        ...
        toggle.addEventListener('change', function () {
            var s = A.getSettings();
            if (!s.genOptions) s.genOptions = {};
            s.genOptions.sourceDetailEnabled = toggle.checked;
            panel.style.display = toggle.checked ? '' : 'none';
            A.saveSetting('genOptions', s.genOptions);
        });

        /* Aspect button clicks */
        panel.addEventListener('click', function (e) {
            var btn = e.target.closest('button[data-aspect]');
            if (!btn) return;
            btn.classList.toggle('active');
            var s = A.getSettings();
            if (!s.genOptions) s.genOptions = {};
            if (!s.genOptions.sourceDetailAspects) s.genOptions.sourceDetailAspects = {};
            s.genOptions.sourceDetailAspects[btn.getAttribute('data-aspect')] = btn.classList.contains('active');
            A.saveSetting('genOptions', s.genOptions);
        });
    }

    /* ══════════════════════════════════════════════
       Era / Setting Tags (시대/배경 태그)
       ══════════════════════════════════════════════ */
    var BUILTIN_ERAS = [
        '고대', '중세', '빅토리안', '1920년대', '1960년대',
        '1990년대', '현대', '근미래', '먼 미래',
        '포스트아포칼립스', '판타지 세계', '우주/SF',
    ];

    function initEraTags() {
        renderEraTags();

        bindClick('auwb-add-era-tag', function () {
            var inp = document.getElementById('auwb-custom-era-input');
            if (!inp) return;
            var val = inp.value.trim();
            if (!val) return;
            ...
            var s = A.getSettings();
            if (!s.genOptions) s.genOptions = {};
            var custom = s.genOptions.customEras || [];
            if (BUILTIN_ERAS.indexOf(val) !== -1 || custom.indexOf(val) !== -1) {
                A.showStatus('이미 존재하는 태그입니다.', 'info');
                return;
            }
            custom.push(val);
            s.genOptions.customEras = custom;
            A.saveSetting('genOptions', s.genOptions);
            inp.value = '';
            renderEraTags();
        });
        ...
        var inp = document.getElementById('auwb-custom-era-input');
        if (inp) inp.addEventListener('keydown', function (e) {
            if (e.key === 'Enter') { e.preventDefault(); document.getElementById('auwb-add-era-tag').click(); }
        });
    }

    function renderEraTags() {
        var container = document.getElementById('auwb-era-tags');
        if (!container) return;

        var s    = A.getSettings();
        var opts = s.genOptions || {};
        var selected = opts.eraTags || [];
        var custom   = opts.customEras || [];
        var allEras  = BUILTIN_ERAS.concat(custom);

        var html = '';
        allEras.forEach(function (era) {
            var isActive = selected.indexOf(era) !== -1;
            var isCustom = custom.indexOf(era) !== -1;
            html += '<button class="auwb-genre-tag auwb-era-tag' + (isActive ? ' active' : '') + '" data-era="' + A.escapeHtml(era) + '">'
                + A.escapeHtml(era)
                + (isCustom ? ' <i class="fa-solid fa-xmark auwb-genre-tag-del auwb-era-tag-del"></i>' : '')
                + '</button>';
        });
        container.innerHTML = html;
        ...
        container.onclick = function (e) {
            /* Delete custom tag */
            if (e.target.classList.contains('auwb-era-tag-del')) {
                var btn = e.target.closest('.auwb-era-tag');
                var era = btn.getAttribute('data-era');
                var gs = A.getSettings();
                if (!gs.genOptions) gs.genOptions = {};
                gs.genOptions.customEras = (gs.genOptions.customEras || []).filter(function (t) { return t !== era; });
                gs.genOptions.eraTags    = (gs.genOptions.eraTags || []).filter(function (t) { return t !== era; });
                A.saveSetting('genOptions', gs.genOptions);
                renderEraTags();
                return;
            }
            var tag = e.target.closest('.auwb-era-tag');
            if (!tag) return;
            var era = tag.getAttribute('data-era');
            tag.classList.toggle('active');
            var gs = A.getSettings();
            if (!gs.genOptions) gs.genOptions = {};
            var cur = gs.genOptions.eraTags || [];
            var idx = cur.indexOf(era);
            if (idx !== -1) cur.splice(idx, 1);
            else cur.push(era);
            gs.genOptions.eraTags = cur;
            A.saveSetting('genOptions', gs.genOptions);
        };
    }
    ...
    /* ══════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════ */
    var BUILTIN_STYLES = [
        '순문학 스타일', '일본 만화 스타일', '라노벨 스타일', '팬픽 스타일',
        '시네마틱 스타일', '하드보일드 스타일', '동화/우화 스타일',
        '고딕 문학 스타일', '웹소설 스타일', '서사시 스타일',
    ];
    ...
    function initStyleTags() {
        ...
        /* Add custom tag */
        bindClick('auwb-add-style-tag', function () {
            var inp = document.getElementById('auwb-custom-style-input');
            if (!inp) return;
            var val = inp.value.trim();
            if (!val) return;
            ...
            var s = A.getSettings();
            if (!s.genOptions) s.genOptions = {};
            var custom = s.genOptions.customStyles || [];
            if (BUILTIN_STYLES.indexOf(val) !== -1 || custom.indexOf(val) !== -1) {
                A.showStatus('이미 존재하는 태그입니다.', 'info');
                return;
            }
            custom.push(val);
            s.genOptions.customStyles = custom;
            A.saveSetting('genOptions', s.genOptions);
            inp.value = '';
            renderStyleTags();
        });
        ...
        var inp = document.getElementById('auwb-custom-style-input');
        if (inp) inp.addEventListener('keydown', function (e) {
            if (e.key === 'Enter') { e.preventDefault(); document.getElementById('auwb-add-style-tag').click(); }
        });
    }

    function renderStyleTags() {
        var container = document.getElementById('auwb-style-tags');
        if (!container) return;

        var s    = A.getSettings();
        var opts = s.genOptions || {};
        var selected = opts.styleTags || [];
        var custom   = opts.customStyles || [];
        var allStyles = BUILTIN_STYLES.concat(custom);

        var html = '';
        allStyles.forEach(function (st) {
            var isActive = selected.indexOf(st) !== -1;
            var isCustom = custom.indexOf(st) !== -1;
            html += '<button class="auwb-genre-tag auwb-style-tag' + (isActive ? ' active' : '') + '" data-style="' + A.escapeHtml(st) + '">'
                + A.escapeHtml(st)
                + (isCustom ? ' <i class="fa-solid fa-xmark auwb-genre-tag-del auwb-style-tag-del"></i>' : '')
                + '</button>';
        });
        container.innerHTML = html;
        ...
        container.onclick = function (e) {
            /* Delete custom tag */
            if (e.target.closest('.auwb-style-tag-del')) {
                var btn = e.target.closest('.auwb-style-tag');
                var style = btn.getAttribute('data-style');
                var gs = A.getSettings();
                if (!gs.genOptions) return;
                gs.genOptions.customStyles = (gs.genOptions.customStyles || []).filter(function (s) { return s !== style; });
                gs.genOptions.styleTags   = (gs.genOptions.styleTags || []).filter(function (s) { return s !== style; });
                A.saveSetting('genOptions', gs.genOptions);
                renderStyleTags();
                return;
            }
            /* Toggle tag */
            var tag = e.target.closest('.auwb-style-tag');
            if (!tag) return;
            var style = tag.getAttribute('data-style');
            tag.classList.toggle('active');
            var gs = A.getSettings();
            if (!gs.genOptions) gs.genOptions = {};
            var cur = gs.genOptions.styleTags || [];
            var idx = cur.indexOf(style);
            if (idx !== -1) cur.splice(idx, 1);
            else cur.push(style);
            gs.genOptions.styleTags = cur;
            A.saveSetting('genOptions', gs.genOptions);
        };
    }
    ...
    A.bindUIEvents = function () {
        ...
        /* Tabs (with lazy init for heavy tabs) */
        var _tabInited = {};
        function _onTabFirstShow(tid) {
            if (tid === 'settings' && _secMgrDirty) {
                A.renderSectionManager();
                A.renderDataStats();
            }
            if (_tabInited[tid]) return;
            _tabInited[tid] = true;
            if (tid === 'settings') {
                A.renderCustomPromptEditors && A.renderCustomPromptEditors();
            } else if (tid === 'presets') {
                A.renderPresetList && A.renderPresetList();
            } else if (tid === 'history') {
                A.renderHistoryList && A.renderHistoryList();
            }
        }
        ...
        bindClick('auwb-diff-mode-toggle', function () {
            _diffSideBySide = !_diffSideBySide;
            var btn = document.getElementById('auwb-diff-mode-toggle');
            if (btn) btn.innerHTML = _diffSideBySide
                ? '<i class="fa-solid fa-list"></i> 통합 보기'
                : '<i class="fa-solid fa-columns"></i> 나란히 보기';
            if (_lastDiffBefore && _lastDiffAfter) _renderDiffContent(_lastDiffBefore, _lastDiffAfter);
        });
        ...
        bindClick('auwb-generate-btn', withLoading('auwb-generate-btn', async function () {
            ...
            /* Self-Critique auto-improvement (A2) */
            var scOpts = (A.getSettings().genOptions || {});
            if (scOpts.selfCritiqueEnabled) {
                try {
                    A.showStatus('자기비평 자동 개선 중…', 'info');
                    var critiqueResult = await A.runSelfCritique();
                    if (critiqueResult.improved > 0) {
                        A.loadChatDataToUI();
                        A.updateExtensionPrompt();
                        A.updateTokenDisplay();
                        A.showStatus('생성 완료! (' + critiqueResult.improved + '개 섹션 자동 개선됨)', 'success');
                    } else {
                        A.showStatus('AU 세계 생성 완료! (비평: 이미 양호)', 'success');
                    }
                } catch (scErr) {
                    A.log('Self-critique error (non-fatal):', scErr);
                    A.showStatus('AU 세계 생성 완료! (자기비평 실패 — 무시됨)', 'success');
                }
            } else {
        ...
        [
            ...
            { b: 'auwb-regen-worldLife',        k: 'worldLife' },
            { b: 'auwb-regen-worldRules',       k: 'worldRules' },
            { b: 'auwb-regen-char',             k: 'charSetting' },
            { b: 'auwb-regen-charPersonality',  k: 'charPersonality' },
            { b: 'auwb-regen-user',             k: 'userSetting' },
            { b: 'auwb-regen-userPersonality',  k: 'userPersonality' },
            { b: 'auwb-regen-charRelation',     k: 'charRelation' },
            { b: 'auwb-regen-charHistory',      k: 'charHistory' },
        ...
        bindClick('auwb-save-world', function () {
            ...
            A.setSectionContent('worldLife', A.getElVal('auwb-world-life-content'));
            A.setSectionContent('worldRules', A.getElVal('auwb-world-rules-content'));
            A.updateExtensionPrompt();
            A.updateTokenDisplay();
        ...
        bindClick('auwb-save-characters', function () {
            ...
            A.setSectionContent('charPersonality', A.getElVal('auwb-char-personality-content'));
            A.setSectionContent('userSetting', A.getElVal('auwb-user-setting-content'));
            A.setSectionContent('userPersonality', A.getElVal('auwb-user-personality-content'));
            A.setSectionContent('charRelation', A.getElVal('auwb-char-relation-content'));
            A.setSectionContent('charHistory', A.getElVal('auwb-char-history-content'));
            A.updateExtensionPrompt();
            A.updateTokenDisplay();
        ...
        /* Consistency Check */
        bindClick('auwb-consistency-check', withLoading('auwb-consistency-check', async function () {
            A.showStatus('일관성 검증 중… (시간이 걸릴 수 있습니다)', 'info');
            try {
                var result = await A.runConsistencyCheck();
                var el = document.getElementById('auwb-consistency-result');
                if (el) el.textContent = result;
                var m = document.getElementById('auwb-consistency-modal');
                if (m) m.style.display = 'flex';
                A.showStatus('일관성 검증 완료!', 'success');
            } catch (err) {
                A.showStatus('일관성 검증 실패: ' + err.message, 'error');
            }
        }));
        bindClick('auwb-consistency-close', function () {
            var m = document.getElementById('auwb-consistency-modal');
            if (m) m.style.display = 'none';
        });
        (function () {
            var ov = document.querySelector('#auwb-consistency-modal .auwb-modal-overlay');
            if (ov) ov.addEventListener('click', function () {
                document.getElementById('auwb-consistency-modal').style.display = 'none';
            });
        })();
        ...
        /* F3: Auto-save (debounced per-section) for section textareas */
        (function () {
            var _autoTimers = {};
            function autoSave(secId, value) {
                if (_autoTimers[secId]) clearTimeout(_autoTimers[secId]);
                _autoTimers[secId] = setTimeout(function () {
                    delete _autoTimers[secId];
            ...
            var builtinMap = {
                ...
                'auwb-world-life-content':       'worldLife',
                'auwb-world-rules-content':      'worldRules',
                'auwb-char-setting-content':     'charSetting',
                'auwb-char-personality-content':  'charPersonality',
                'auwb-user-setting-content':     'userSetting',
                'auwb-user-personality-content':  'userPersonality',
                'auwb-char-relation-content':     'charRelation',
                'auwb-char-history-content':      'charHistory',
            ...
            if (genreEl) genreEl.addEventListener('input', function () {
                if (_autoTimers._genre) clearTimeout(_autoTimers._genre);
                _autoTimers._genre = setTimeout(function () {
                    delete _autoTimers._genre;
            ...
            if (conceptEl) conceptEl.addEventListener('input', function () {
                if (_autoTimers._concept) clearTimeout(_autoTimers._concept);
                _autoTimers._concept = setTimeout(function () {
                    delete _autoTimers._concept;
        ...
        /* ── Concept Idea Modal ── */
        (function () {
            var ideaModal = document.getElementById('auwb-idea-modal');
            if (!ideaModal) return;

            /* Open / Close */
            bindClick('auwb-open-idea', function () { ideaModal.style.display = 'flex'; _renderLibrary(); });
            bindClick('auwb-idea-close', function () { ideaModal.style.display = 'none'; });
            var ov = ideaModal.querySelector('.auwb-modal-overlay');
            if (ov) ov.addEventListener('click', function () { ideaModal.style.display = 'none'; });

            /* Tab switching */
            ideaModal.querySelectorAll('.auwb-idea-tab').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    ideaModal.querySelectorAll('.auwb-idea-tab').forEach(function (b) { b.classList.remove('active'); });
                    btn.classList.add('active');
                    var tid = btn.getAttribute('data-idea-tab');
                    ideaModal.querySelectorAll('.auwb-idea-panel').forEach(function (p) { p.style.display = 'none'; });
                    var panel = document.getElementById('auwb-idea-panel-' + tid);
                    if (panel) panel.style.display = 'block';
                    if (tid === 'custom') _renderSlotEditor();
                });
            });
            ...
            function _renderLibrary() {
                var lib = A.CONCEPT_LIBRARY;
                var catEl = document.getElementById('auwb-idea-cat-list');
                var smpEl = document.getElementById('auwb-idea-sample-list');
                if (!catEl || !smpEl) return;
                catEl.innerHTML = lib.map(function (c, i) {
                    return '<button class="auwb-idea-cat' + (i === _activeCat ? ' active' : '') + '" data-ci="' + i + '">' + A.escapeHtml(c.cat) + '</button>';
                }).join('');
                _renderSamples();
                catEl.onclick = function (e) {
                    var btn = e.target.closest('.auwb-idea-cat');
                    if (!btn) return;
                    _activeCat = parseInt(btn.getAttribute('data-ci'));
                    catEl.querySelectorAll('.auwb-idea-cat').forEach(function (b) { b.classList.remove('active'); });
                    btn.classList.add('active');
                    _renderSamples();
                };
            }
            function _renderSamples() {
                var smpEl = document.getElementById('auwb-idea-sample-list');
                var items = (A.CONCEPT_LIBRARY[_activeCat] || {}).items || [];
                smpEl.innerHTML = items.map(function (it) {
                    return '<div class="auwb-idea-item">' + A.escapeHtml(it) + '</div>';
                }).join('');
                smpEl.onclick = function (e) {
                    var item = e.target.closest('.auwb-idea-item');
                    if (!item) return;
                    _applyToConceptArea(item.textContent);
                };
            }

            /* === Combo Panel === */
            function _rand(arr) { return arr[Math.floor(Math.random() * arr.length)]; }
            bindClick('auwb-combo-roll', function () {
                document.getElementById('auwb-combo-setting').textContent  = _rand(_getAllSlotItems(_slotMap.settings));
                document.getElementById('auwb-combo-relation').textContent = _rand(_getAllSlotItems(_slotMap.relations));
                document.getElementById('auwb-combo-twist').textContent    = _rand(_getAllSlotItems(_slotMap.twists));
            });
            bindClick('auwb-combo-use', function () {
                var s = document.getElementById('auwb-combo-setting').textContent;
                var r = document.getElementById('auwb-combo-relation').textContent;
                var t = document.getElementById('auwb-combo-twist').textContent;
                if (s === '\u2014') { A.showStatus('\uba3c\uc800 \ub79c\ub364 \ub3cc\ub9ac\uae30\ub97c \ub20c\ub7ec\uc8fc\uc138\uc694.', 'info'); return; }
                _applyToConceptArea('[' + s + '] \u00d7 [' + r + '] \u00d7 [' + t + ']');
            });

            /* === Slot Editor Panel (요소 편집) === */
            var _slotMap = {
                settings:  { builtInKey: 'COMBO_SETTINGS',  userKey: 'userComboSettings',  deletedKey: 'deletedComboSettings',  label: '배경' },
                relations: { builtInKey: 'COMBO_RELATIONS', userKey: 'userComboRelations', deletedKey: 'deletedComboRelations', label: '관계' },
                twists:    { builtInKey: 'COMBO_TWISTS',    userKey: 'userComboTwists',    deletedKey: 'deletedComboTwists',    label: '반전 요소' },
            };

            function _getSlotItems(info) {
                var s = A.getSettings();
                var deleted = s[info.deletedKey] || [];
                var builtIn = A[info.builtInKey].filter(function (it) { return deleted.indexOf(it) === -1; });
                var user = s[info.userKey] || [];
                return { builtIn: builtIn, user: user };
            }

            function _getAllSlotItems(info) {
                var items = _getSlotItems(info);
                return items.builtIn.concat(items.user);
            }

            function _renderSlotEditor() {
                var sel = document.getElementById('auwb-slot-editor-select');
                var listEl = document.getElementById('auwb-slot-editor-list');
                if (!sel || !listEl) return;
                var key = sel.value;
                var info = _slotMap[key];
                if (!info) return;
                var items = _getSlotItems(info);
                var html = '';
                items.builtIn.forEach(function (it) {
                    html += '<div class="auwb-idea-item auwb-slot-item auwb-slot-builtin" data-val="' + A.escapeHtml(it) + '">'
                        + '<span class="auwb-idea-custom-text">' + A.escapeHtml(it) + '</span>'
                        + '<button class="auwb-idea-del" title="삭제"><i class="fa-solid fa-trash-can"></i></button>'
                        + '</div>';
                });
                items.user.forEach(function (it, i) {
                    html += '<div class="auwb-idea-item auwb-slot-item auwb-slot-user" data-idx="' + i + '">'
                        + '<span class="auwb-idea-custom-text">' + A.escapeHtml(it) + '</span>'
                        + '<button class="auwb-idea-del" title="삭제"><i class="fa-solid fa-trash-can"></i></button>'
                        + '</div>';
                });
                if (!items.builtIn.length && !items.user.length) {
                    html = '<div class="auwb-preset-empty">항목이 없습니다.</div>';
                }
                listEl.innerHTML = html;
                listEl.onclick = function (e) {
                    if (e.target.closest('.auwb-idea-del')) {
                        var userEl = e.target.closest('.auwb-slot-user');
                        if (userEl) {
                            var idx = parseInt(userEl.getAttribute('data-idx'));
                            var st = A.getSettings();
                            var arr = st[info.userKey] || [];
                            arr.splice(idx, 1);
                            A.saveSetting(info.userKey, arr);
                            _renderSlotEditor();
                            return;
                        }
                        var biEl = e.target.closest('.auwb-slot-builtin');
                        if (biEl) {
                            var val = biEl.getAttribute('data-val');
                            var st2 = A.getSettings();
                            if (!Array.isArray(st2[info.deletedKey])) st2[info.deletedKey] = [];
                            st2[info.deletedKey].push(val);
                            A.saveSetting(info.deletedKey, st2[info.deletedKey]);
                            _renderSlotEditor();
                            return;
                        }
                    }
            ...
            var slotSel = document.getElementById('auwb-slot-editor-select');
            if (slotSel) slotSel.addEventListener('change', _renderSlotEditor);

            bindClick('auwb-slot-editor-add', function () {
                var inp = document.getElementById('auwb-slot-editor-input');
                var val = (inp && inp.value || '').trim();
                if (!val) return;
                var sel = document.getElementById('auwb-slot-editor-select');
                var info = _slotMap[sel.value];
                if (!info) return;
                var s = A.getSettings();
                if (!Array.isArray(s[info.userKey])) s[info.userKey] = [];
                s[info.userKey].push(val);
                A.saveSetting(info.userKey, s[info.userKey]);
                inp.value = '';
                _renderSlotEditor();
                A.showStatus(info.label + ' 요소 추가됨!', 'success');
            });
            var slotInp = document.getElementById('auwb-slot-editor-input');
            if (slotInp) slotInp.addEventListener('keydown', function (e) {
                if (e.key === 'Enter') { e.preventDefault(); document.getElementById('auwb-slot-editor-add').click(); }
            });

            /* Slot import/export */
            bindClick('auwb-slot-export', function () {
                var data = {};
                Object.keys(_slotMap).forEach(function (k) {
                    var info = _slotMap[k];
                    var s = A.getSettings();
                    data[k] = { user: s[info.userKey] || [], deleted: s[info.deletedKey] || [] };
                });
                var blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
                var a = document.createElement('a');
                a.href = URL.createObjectURL(blob);
                a.download = 'auwb-combo-slots.json';
                a.click();
                URL.revokeObjectURL(a.href);
                A.showStatus('요소 리스트 내보내기 완료!', 'success');
            });
            bindClick('auwb-slot-import', function () {
                document.getElementById('auwb-slot-import-file').click();
            });
            var slotFile = document.getElementById('auwb-slot-import-file');
            if (slotFile) slotFile.addEventListener('change', function (e) {
                ...
                reader.onload = function (ev) {
                    try {
                        ...
                        Object.keys(_slotMap).forEach(function (k) {
                            var info = _slotMap[k];
                            if (data[k]) {
                                if (Array.isArray(data[k].user)) { s[info.userKey] = data[k].user; A.saveSetting(info.userKey, s[info.userKey]); }
                                if (Array.isArray(data[k].deleted)) { s[info.deletedKey] = data[k].deleted; A.saveSetting(info.deletedKey, s[info.deletedKey]); }
                            }
                        });
                        _renderSlotEditor();
                        A.showStatus('요소 리스트 불러오기 완료!', 'success');
                    } catch (err) {
                        A.showStatus('파일 형식 오류', 'error');
                    }
                };
                reader.readAsText(f);
                e.target.value = '';
            ...
            /* === Expand Modal (textarea 확대 팝업) === */
            (function () {
                var expandModal = document.getElementById('auwb-expand-modal');
                var expandTA = document.getElementById('auwb-expand-textarea');
                var expandTitle = document.getElementById('auwb-expand-title');
                if (!expandModal || !expandTA) return;
                var _sourceTA = null;

                function openExpand(sourceTextarea, title) {
                    _sourceTA = sourceTextarea;
                    expandTA.value = sourceTextarea.value;
                ...
                function saveExpand() {
                    if (_sourceTA) {
                        _sourceTA.value = expandTA.value;
                        _sourceTA.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                    closeExpand();
                }

                /* Delegate all expand buttons (use document — popup is dynamically loaded) */
                document.addEventListener('click', function (e) {
                    var btn = e.target.closest('.auwb-expand-btn');
                    if (!btn) return;
                    e.preventDefault();
                    e.stopPropagation();
                    var targetId = btn.getAttribute('data-target-ta');
                    var ta = targetId && document.getElementById(targetId);
                    if (!ta) return;
                    /* Derive title: data attr → closest h4 text → generic */
                    var title = btn.getAttribute('data-expand-title') || '';
                    if (!title) {
                        var sec = btn.closest('.auwb-section');
                        var h4 = sec && sec.querySelector('h4');
                        if (h4) title = h4.textContent.trim();
                    }
                    openExpand(ta, title || '내용 편집');
                });

                bindClick('auwb-expand-save', saveExpand);
                bindClick('auwb-expand-close', closeExpand);
                /* Close on overlay or modal background click */
                expandModal.addEventListener('click', function (e) {
                    if (e.target === expandModal || e.target.classList.contains('auwb-modal-overlay')) closeExpand();
                });
            })();

            /* === Selective Partial Regeneration (floating button) === */
            (function () {
                /* Build reverse map: textarea-id → section-id */
                var taToSec = {};
                Object.keys(A._biTextareaMap).forEach(function (sid) {
                    taToSec[A._biTextareaMap[sid]] = sid;
                });

                /* Create floating button */
                var floatBtn = document.createElement('button');
                floatBtn.className = 'auwb-btn auwb-btn-accent auwb-btn-sm auwb-partial-regen-float';
                floatBtn.innerHTML = '<i class="fa-solid fa-wand-sparkles"></i> 부분 재생성';
                floatBtn.style.display = 'none';
                document.body.appendChild(floatBtn);

                var _activeTa = null, _selectedText = '';

                function getSectionIdFromTextarea(ta) {
                    if (!ta) return null;
                    if (ta.id && taToSec[ta.id]) return taToSec[ta.id];
                    var csid = ta.getAttribute('data-custom-section-id');
                    if (csid) return csid;
                    return null;
                }

                function showFloat(ta) {
                    var sel = ta.value.substring(ta.selectionStart, ta.selectionEnd).trim();
                    if (!sel || sel.length < 5) { floatBtn.style.display = 'none'; return; }
                    _activeTa = ta;
                    _selectedText = sel;
                    var rect = ta.getBoundingClientRect();
                    floatBtn.style.position = 'fixed';
                    floatBtn.style.top = Math.max(0, rect.top - 36) + 'px';
                    floatBtn.style.left = (rect.left + rect.width / 2 - 60) + 'px';
                    floatBtn.style.zIndex = '999999';
                    floatBtn.style.display = '';
                }

                document.addEventListener('mouseup', function (e) {
                    var ta = e.target.closest && e.target.closest('textarea.auwb-textarea');
                    if (!ta) { if (e.target !== floatBtn && !floatBtn.contains(e.target)) floatBtn.style.display = 'none'; return; }
                    var sid = getSectionIdFromTextarea(ta);
                    if (!sid) return;
                    setTimeout(function () { showFloat(ta); }, 10);
                });

                floatBtn.addEventListener('click', async function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    if (!_activeTa || !_selectedText) return;
                    var sid = getSectionIdFromTextarea(_activeTa);
                    if (!sid) return;
                    var instruction = await A.showPrompt('부분 재생성 지시사항 (선택)', '선택한 부분을 어떻게 변경할까요?');
                    floatBtn.style.display = 'none';
                    A.showStatus('부분 재생성 중…', 'info');
                    try {
                        A.saveToHistory('regen', A.getFullSnapshot());
                        var res = await A.regenerateSectionPartial(sid, _selectedText, instruction || '');
                        if (res && res.value) {
                            A.commitSectionChange(sid, res.value);
                            A.showStatus('부분 재생성 완료!', 'success');
                        }
                    } catch (err) {
                        A.showStatus('부분 재생성 실패: ' + err.message, 'error');
                    }
                });
            })();

            /* Shared: apply text to concept textarea */
            function _applyToConceptArea(text) {
                var ta = document.getElementById('auwb-au-concept');
                if (!ta) return;
                ta.value = text;
                ideaModal.style.display = 'none';
                ta.focus();
                A.showStatus('컨셉 적용됨!', 'success');
            }
        })();

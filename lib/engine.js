...
(function (A) {
    ...
    A.callAPI = async function (prompt) {
        ...
        if (settings.apiSource === 'openai') {
            ...
            var tid  = setTimeout(function () { ctrl.abort(); }, (settings.customApiTimeout || 600) * 1000);
            try {
                var resp = await fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(body), signal: ctrl.signal });
                if (!resp.ok) {
                    var httpError = new Error('HTTP ' + resp.status + ': ' + (await resp.text()).substring(0, 200));
                ...
                if (data.choices && data.choices[0] && data.choices[0].finish_reason === 'length') {
                    var truncated = new Error('응답이 토큰 한도로 잘렸습니다. 출력 분량을 줄이거나 최대 토큰을 늘려 주세요.');
                    truncated.noRetry = true;
                    throw truncated;
                }
                if (typeof result !== 'string' || !result.trim()) throw new Error('Empty response from custom API');
                return result;
        ...
        try {
            ...
            /* F13: Timeout for SillyTavern API — generous default for large generations */
            var stTimeout = (settings.customApiTimeout || 600) * 1000;
    ...
    A.callAPIWithRetry = async function (prompt, retries) {
        ...
        for (var attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                lastErr = e;
                if (e.noRetry || (e.status >= 400 && e.status < 500 && e.status !== 408 && e.status !== 429)) break;
                /* Do NOT retry on timeout or abort — it will just fail again */
                var msg = (e.message || '').toLowerCase();
                if (msg.indexOf('시간 초과') !== -1 || msg.indexOf('timeout') !== -1 || msg.indexOf('abort') !== -1) {
                    A.log('Timeout/abort detected — skipping retries');
                    break;
                }
    ...
    A.getPromptTemplate = function (key) {
        ...
        var v  = cp[key];
        if (v && v.trim()) {
            // Older saved initial templates lacked the context slot entirely.
            if (key === 'initial' && v.indexOf('{{EXISTING_SETTINGS}}') === -1) {
                return v + '\n\n{{EXISTING_SETTINGS}}';
            }
            return v;
        }
    ...
    A.buildGuidelines = function () {
        ...
        g.push(opts.cliche === 'subvert'
            ? '- SUBVERT CLICHES: Change a concrete cause, incentive or consequence of a familiar trope; a surprise alone is not originality.'
            : '- CLICHES ALLOWED: Classic tropes are fine.');

        /* Original — 4 levels */
        var origMap = {
            break:  '- FULL REIMAGINING: Use only the character\'s name and general role. DISCARD all specific traits, abilities, and physical details. Build entirely new personalities and backstories from the AU premise.',
            loose:  '- LOOSE INSPIRATION: Keep the general "feel" of characters but freely change specific traits and details.',
            core:   '- CORE TRAITS ONLY: Preserve essential personality pillars but adapt surface details to the AU.',
            keep:   '- FAITHFUL PRESERVATION: Keep all major traits and relationships intact. Adapt only what the AU requires.',
        };
        g.push(origMap[opts.original] || origMap.loose);

        /* Source detail aspects — fine-grained control */
        if (opts.sourceDetailEnabled && opts.sourceDetailAspects) {
            var asp = opts.sourceDetailAspects;
            var include = [], exclude = [];
            var labels = {
                name: 'name', personality: 'personality/temperament',
                age: 'age', appearance: 'physical appearance/looks',
                abilities: 'abilities/skills/powers',
                background: 'backstory/history/background',
                relationships: 'canon relationships/dynamics'
            };
            Object.keys(labels).forEach(function (k) {
                if (asp[k]) include.push(labels[k]);
                else exclude.push(labels[k]);
            });
            if (include.length) g.push('- SOURCE DETAIL FILTER (INCLUDE): From the original character data, actively use and reflect these aspects: ' + include.join(', ') + '.');
            if (exclude.length) g.push('- SOURCE DETAIL FILTER (EXCLUDE): Do NOT carry over these aspects from the source material — reinvent them freely for the AU: ' + exclude.join(', ') + '.');
        }

        /* Relation — 5 levels */
        var relMap = {
            first:    '- FIRST MEETING: ' + ci.charName + ' and ' + ci.userName + ' are strangers.',
            ...
            complex:  '- COMPLEX HISTORY: A complicated, layered past (rivalry, ex-friends, etc.).',
        };
        g.push(relMap[opts.relation] || relMap.first);

        /* Mood — 5 options */
        var moodMap = {
            humorous:    '- HUMOROUS: Playful, witty, comedic beats. Lean into absurdity and banter.',
            light:       '- LIGHT: Bright, heartwarming tone.',
            bittersweet: '- BITTERSWEET: Warmth mixed with melancholy.',
            dark:        '- DARK: Serious, dramatic, intense.',
            grim:        '- GRIM: Unforgiving, raw, visceral. No comfort without cost.',
        };
        ...
        /* Style Tags */
        if (opts.styleTags && opts.styleTags.length) {
            g.push('- WRITING STYLE: ' + opts.styleTags.join(', ') + '.');
        }

        /* Era / Setting Tags */
        if (opts.eraTags && opts.eraTags.length) {
            g.push('- ERA / SETTING: ' + opts.eraTags.join(', ') + '.');
        }

        /* Theme Anchor */
        if (opts.themeAnchor && opts.themeAnchor.trim()) {
            g.push('- CENTRAL MOTIF: Weave "' + opts.themeAnchor.trim() + '" throughout all sections as a recurring theme.');
        }

        /* Dramatic Question */
        if (opts.dramaticQuestion && opts.dramaticQuestion.trim()) {
            g.push('- DRAMATIC QUESTION: The AU revolves around — "' + opts.dramaticQuestion.trim() + '". Every section should create tension around this.');
        }

        /* Must-Include */
        if (opts.mustInclude && opts.mustInclude.trim()) {
            g.push('- MUST INCLUDE: ' + opts.mustInclude.trim());
        }

        /* Must-Exclude */
        if (opts.mustExclude && opts.mustExclude.trim()) {
            g.push('- MUST AVOID: ' + opts.mustExclude.trim());
        }

        /* Playable specificity, without prescribing the same twist every time. */
        g.push('- PRIORITY: Follow the user concept, exclusions, source-preservation level, relationship and mood. These override general creative suggestions. Familiar tropes are welcome when requested.');
        g.push('- DESIGN: Choose one concrete organizing idea for this AU and show its consequences in work, routines and relationships. Avoid piling on unrelated lore, factions or special powers.');
        g.push('- CHARACTER: Within the selected preservation level, translate recognizable motives and habits into AU-specific behavior, decisions and limitations. Changing only a job title is insufficient. Do not give everyone the same guarded exterior and secretly tender heart.');
        g.push('- PLAYABILITY: Give the characters a plausible reason to interact and a present situation with room for different responses. Leave outcomes, attraction and player choices open. Low-stakes everyday wants work in light or peaceful settings; do not force danger or conflict.');
        g.push('- SPECIFICITY: Prefer a useful local custom, practical constraint, object in use or observable habit over abstract claims about destiny, secrets and a complex past. Details should affect behavior; avoid repetitive atmospheric padding.');
        g.push('- COHERENCE: Each section adds distinct information. Reuse established facts rather than retelling them. Strangers have no shared history unless explicitly requested; describe separate paths toward their first encounter instead. Clothing need not conceal a secret.');
        g.push('- FINAL CHECK: Before returning only the requested content, remove interchangeable descriptions, unsupported canon changes, repeated exposition and forced twists. Match the selected tone and length.');
    ...
    A.buildOutputFormat = function (filterIds) {
        ...
        return sections.map(function (sec) {
            ...
            switch (sec.id) {
                case 'world':
                    desc = '(' + worldP + 'P: world overview — setting, geography, atmosphere, tensions)';
                    break;
                case 'worldLife':
                    desc = '(' + Math.max(1, worldP - 1) + 'P: daily life & culture)';
                    break;
                case 'worldRules':
                    desc = '(' + Math.max(1, worldP - 1) + 'P: special rules & systems)';
                    break;
                case 'charSetting':
                    desc = '(' + charP + 'P: ' + ci.charName + ' — AU role, occupation, circumstances)';
                    break;
                case 'charPersonality':
                    desc = '(' + charP + 'P: ' + ci.charName + ' — motives, observable habits, strengths and limitations in this AU)';
                    break;
                case 'userSetting':
                    desc = '(' + charP + 'P: ' + ci.userName + ' — AU role, occupation, circumstances)';
                    break;
                case 'userPersonality':
                    desc = '(' + charP + 'P: ' + ci.userName + ' — personality, stakes, dynamic with ' + ci.charName + ')';
                    break;
                case 'charRelation':
                    desc = '(' + charP + 'P: relationship between ' + ci.charName + ' & ' + ci.userName + ' — dynamic, tensions, perception)';
                    break;
                case 'charHistory':
                    desc = '(' + charP + 'P: relevant past (no shared past if strangers) of ' + ci.charName + ' & ' + ci.userName + ')';
                    break;
                case 'charClothing':
                    desc = '(' + clothP + 'P: ' + ci.charName + "'s signature look)";
                    break;
                case 'userClothing':
                    desc = '(' + clothP + 'P: ' + ci.userName + "'s attire)";
                    break;
                default:
                    desc = '(Content for: ' + sec.label + ')';
                    break;
            }
            /* Per-section custom directive (B6) */
            var directives = opts.sectionDirectives || {};
            if (directives[sec.id] && directives[sec.id].trim()) {
                desc += ' [USER DIRECTIVE: ' + directives[sec.id].trim() + ']';
            }
            return '[' + sec.tag + ']\n' + desc + '\n[/' + sec.tag + ']';
        }).join('\n\n');
    };
    ...
    /** When original='break', strip detailed character info to prevent trait fixation.
     *  When sourceDetail is enabled, selectively mask aspects based on user choices. */
    A.getCharInfoForPrompt = function () {
        ...
        if (opts.original === 'break') {
            return {
                ...
                charDesc:  '(Reimagining mode — use only the name. Invent all traits from scratch for the AU.)',
                charPers:  '(Reimagining mode — create entirely new personality for the AU.)',
                charScene: '(Reimagining mode — ignore source scenario.)',
                userName:  ci.userName,
                userPersona: ci.personaDescription ? '(Reimagining mode — use only as minimal reference.)' : 'Not provided',
            };
        }
        var result = {
            charName:    ci.charName,
            charDesc:    ci.charDescription || 'Not provided',
            charPers:    ci.charPersonality || 'Not provided',
            charScene:   ci.charScenario    || 'Not provided',
            userName:    ci.userName,
            userPersona: ci.personaDescription || 'Not provided',
        };
        /* Fine-grained source detail filtering */
        if (opts.sourceDetailEnabled && opts.sourceDetailAspects) {
            var asp = opts.sourceDetailAspects;
            if (!asp.appearance && !asp.abilities && !asp.background) {
                result.charDesc = '(Source detail filter active — description excluded. Reinvent freely for the AU.)';
            }
            if (!asp.personality) {
                result.charPers = '(Source detail filter active — personality excluded. Create new personality for the AU.)';
            }
            if (!asp.background) {
                result.charScene = '(Source detail filter active — scenario/background excluded. Reinvent freely.)';
            }
        }
        return result;
    };
    ...
    A.generateAUWorld = async function (concept, filterIds) {
        ...
        /* Dispatch based on advanced generation modes */
        if (opts.sequentialEnabled) {
            return await A._generateSequential(concept, filterIds);
        }
        if (opts.twoPassEnabled) {
            return await A._generateTwoPass(concept, filterIds);
        }
        return await A._generateDirect(concept, filterIds);
    };

    /** Standard single-pass generation */
    A._generateDirect = async function (concept, filterIds) {
        ...
        return await A.callAPIWithRetry(A.fillTemplate(A.getPromptTemplate('initial'), {
            ...
            CHAR_NAME:          pi.charName,
            CHAR_DESC:          pi.charDesc,
            CHAR_PERS:          pi.charPers,
            CHAR_SCENE:         pi.charScene,
            USER_NAME:          pi.userName,
            USER_PERSONA:       pi.userPersona,
    ...
    /** 2-Pass: skeleton outline → full prose using skeleton as context */
    A._generateTwoPass = async function (concept, filterIds) {
        var pi  = A.getCharInfoForPrompt();
        var cd  = A.getChatData();
        var ref = cd.reference || '';
        var rel = cd.relationship || '';
        ...
        /* Pass 1: skeleton */
        A.showStatus('2-Pass: 스켈레톤 생성 중… (1/2)', 'info');
        var skeletonVars = Object.assign({}, commonVars, {
            OUTPUT_FORMAT: A.buildOutputFormat(filterIds),
        });
        var skeleton = await A.callAPIWithRetry(A.fillTemplate(A.DEFAULT_SKELETON_PROMPT, skeletonVars));
        if (!skeleton) throw new Error('Skeleton generation failed');

        /* Pass 2: flesh out using skeleton as EXISTING_SETTINGS context */
        A.showStatus('2-Pass: 본문 생성 중… (2/2)', 'info');
        var fleshVars = Object.assign({}, commonVars, {
            OUTPUT_FORMAT:      A.buildOutputFormat(filterIds),
            VOLUME_INSTRUCTION: A.getVolumeInstruction(),
            EXISTING_SETTINGS:  '\n## AU SKELETON (follow this structure and expand into full prose)\n' + skeleton,
        });
        return await A.callAPIWithRetry(A.fillTemplate(A.getPromptTemplate('initial'), fleshVars));
    };

    /** Sequential pipeline: world sections → character sections → relationship sections */
    A._generateSequential = async function (concept, filterIds) {
        var pi  = A.getCharInfoForPrompt();
        var cd  = A.getChatData();
        var ref = cd.reference || '';
        var rel = cd.relationship || '';
        ...
        /* Group sections into phases */
        var allIds = filterIds || A.getEnabledSections().map(function (s) { return s.id; });
        var phases = [
            { label: '세계관', ids: ['world', 'worldLife', 'worldRules'] },
            { label: '캐릭터', ids: ['charSetting', 'charPersonality', 'userSetting', 'userPersonality'] },
            { label: '관계/기타', ids: ['charRelation', 'charHistory', 'charClothing', 'userClothing'] },
        ];
        ...
        for (var p = 0; p < phases.length; p++) {
            var phase = phases[p];
            var phaseIds = phase.ids.filter(function (id) { return allIds.indexOf(id) !== -1; });
            if (!phaseIds.length) continue;

            A.showStatus('순차 생성: ' + phase.label + ' (' + (p + 1) + '/' + phases.length + ')', 'info');

            /* Build existing settings from already-generated phases */
            var existCtx = '';
            if (Object.keys(accumulated).length) {
                var lines = [];
                A.getAllSections().forEach(function (sec) {
                    if (accumulated[sec.id]) lines.push('### ' + sec.label + ': ' + accumulated[sec.id]);
                });
                existCtx = lines.join('\n');
            }

            var phaseVars = Object.assign({}, commonVars, {
                OUTPUT_FORMAT:     A.buildOutputFormat(phaseIds),
                EXISTING_SETTINGS: existCtx ? '\n## Already Established Sections (maintain consistency)\n' + existCtx : '',
            });
            var rawResult = await A.callAPIWithRetry(A.fillTemplate(A.getPromptTemplate('initial'), phaseVars));
            if (rawResult) {
                allRaw.push(rawResult);
                var parsed = A.parseGeneratedContent(rawResult);
                Object.keys(parsed).forEach(function (k) { accumulated[k] = parsed[k]; });
            }
        }

        /* Also handle custom sections that didn't fit into standard phases */
        var coveredIds = [];
        phases.forEach(function (ph) { coveredIds = coveredIds.concat(ph.ids); });
        var customIds = allIds.filter(function (id) { return coveredIds.indexOf(id) === -1; });
        if (customIds.length) {
            A.showStatus('순차 생성: 커스텀 섹션…', 'info');
            var cLines = [];
            A.getAllSections().forEach(function (sec) {
                if (accumulated[sec.id]) cLines.push('### ' + sec.label + ': ' + accumulated[sec.id]);
            });
            var cVars = Object.assign({}, commonVars, {
                OUTPUT_FORMAT:     A.buildOutputFormat(customIds),
                EXISTING_SETTINGS: cLines.length ? '\n## Already Established Sections\n' + cLines.join('\n') : '',
            });
            var cRaw = await A.callAPIWithRetry(A.fillTemplate(A.getPromptTemplate('initial'), cVars));
            if (cRaw) allRaw.push(cRaw);
        }

        return allRaw.join('\n\n');
    };

    /** Self-Critique: analyze generated content, find weak sections, auto-regenerate them */
    A.runSelfCritique = async function () {
        var existing = A.buildExistingSettings();
        if (!existing || existing.indexOf('(Not yet established)') !== -1) {
            throw new Error('먼저 AU를 생성해주세요.');
        }

        A.showStatus('자기비평 분석 중…', 'info');
        var critiqueResult = await A.callAPIWithRetry(A.fillTemplate(A.DEFAULT_SELF_CRITIQUE_PROMPT, {
            ALL_SETTINGS:     existing,
            LANG_INSTRUCTION: A.getLangInstruction(),
        }));

        if (!critiqueResult) throw new Error('Empty critique response');
        if (critiqueResult.indexOf('[NO_ISSUES]') !== -1) {
            return { issues: [], improved: 0 };
        }

        /* Parse critique blocks */
        var issues = [];
        for (var i = 1; i <= 3; i++) {
            var re = new RegExp('\\[CRITIQUE_' + i + '\\]([\\s\\S]*?)\\[\\/CRITIQUE_' + i + '\\]', 'i');
            var m  = critiqueResult.match(re);
            if (m) {
                var text = m[1].trim();
                var secM = text.match(/Section:\s*(.+)/i);
                var probM = text.match(/Problem:\s*(.+)/i);
                var fixM  = text.match(/Fix:\s*(.+)/i);
                if (secM) {
                    issues.push({
                        section: secM[1].trim().toLowerCase().replace(/\s+/g, ''),
        ...
        /* Auto-regenerate weak sections */
        var improved = 0;
        for (var j = 0; j < issues.length; j++) {
            var issue = issues[j];
            /* Map section name to valid id */
            var secId = issue.section;
            var found = A.getAllSections().find(function (sec) {
                return sec.id.toLowerCase() === secId || sec.tag.toLowerCase() === secId;
            });
            if (!found || !A.isSectionEnabled(found.id) || A.isSectionLocked(found.id)) continue;

            A.showStatus('약한 섹션 개선 중: ' + found.label + ' (' + (j + 1) + '/' + issues.length + ')', 'info');
            try {
                var result = await A.refineSection(found.id, 'specific');
                if (result && result.value) {
                    A.setSectionContent(found.id, result.value);
                    improved++;
                }
    ...
    A.regenerateSection = async function (sectionKey) {
        ...
        var result = await A.callAPIWithRetry(A.fillTemplate(A.getPromptTemplate('section'), {
            ...
            CHAR_DESC:         ci.charDesc || '',
            USER_NAME:         ci.userName,
            USER_PERSONA:      ci.userPersona || '',
    ...
    A.applyUpdateResult = function (result, type) {
        if (/^\s*\[NO_CHANGES\]\s*$/i.test(result || '')) return;

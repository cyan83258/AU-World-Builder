...
(function (A) {
    ...
    A.buildGuidelines = function () {
        ...
        /* Original — 4 levels */
        var origMap = {
            break:  '- FULL REIMAGINING: Completely reinvent characters from scratch for this AU. The source character sheet is ONLY a loose silhouette — use it to know the character\'s name and general role, then DISCARD all specific traits, abilities, physical details, and quirks. Do NOT recycle, echo, or riff on any particular attribute from the source (e.g., if a character has good hearing, do NOT give them audio-related hobbies, sound powers, or acoustic themes). Build entirely new personalities, skills, habits, and backstories that arise organically from the AU premise. The result should feel like an original character who merely shares a name.',
            loose:  '- LOOSE INSPIRATION: Use the original characters as a mood/vibe reference. Keep the general "feel" (e.g., a warm-hearted character stays warm-hearted), but freely change specific traits, habits, and details. Do NOT copy individual characteristics verbatim or build AU details around any single source trait.',
            core:   '- CORE TRAITS ONLY: Preserve the essential personality pillars (e.g., kind, stubborn, ambitious) but adapt surface details, habits, and quirks to fit the AU naturally. Avoid fixating on minor or physical traits from the source.',
        ...
        /* Mood — expanded 5 options */
        var moodMap = {
            humorous:    '- HUMOROUS ATMOSPHERE: Playful, witty, comedic beats throughout. Lean into absurdity, banter, and comic timing.',
            ...
            grim:        '- GRIM/CRUEL ATMOSPHERE: Unforgiving, raw, visceral. Consequences are real and permanent. No comfort without cost.',
        };
        ...
        if (opts.styleTags && opts.styleTags.length) {
            g.push('- WRITING STYLE: Adopt the narrative conventions of — ' + opts.styleTags.join(', ') + '. Let this influence prose rhythm, description density, dialogue style, and dramatic pacing.');
        }

        /* Era / Setting Tags */
        if (opts.eraTags && opts.eraTags.length) {
            g.push('- ERA / SETTING: Ground the world in — ' + opts.eraTags.join(', ') + '. Let this era shape technology, social norms, architecture, language register, and daily life details.');
        }

        /* Theme Anchor (central motif) */
        if (opts.themeAnchor && opts.themeAnchor.trim()) {
            g.push('- CENTRAL MOTIF: Weave the theme/symbol of "' + opts.themeAnchor.trim() + '" throughout ALL sections as a recurring motif. Each section should echo or refract this theme in a different way — through imagery, character behavior, world rules, or relationship dynamics.');
        }

        /* Dramatic Question */
        if (opts.dramaticQuestion && opts.dramaticQuestion.trim()) {
            g.push('- DRAMATIC QUESTION: The entire AU revolves around this core question — "' + opts.dramaticQuestion.trim() + '". Every section should create tension around this question. Characters\' actions, world rules, and relationships should all push toward or pull away from answering it.');
        }

        /* Must-Include */
        if (opts.mustInclude && opts.mustInclude.trim()) {
            g.push('- MUST INCLUDE: The following elements MUST appear naturally in the AU — ' + opts.mustInclude.trim() + '. Integrate them organically, not as forced additions.');
        }

        /* Must-Exclude */
        if (opts.mustExclude && opts.mustExclude.trim()) {
            g.push('- MUST AVOID: Do NOT include any of the following — ' + opts.mustExclude.trim() + '. Find alternative approaches if needed.');
        }
        ...
        g.push('- ORIGINALITY: Avoid textbook-style descriptions, Wikipedia-tone summaries, and overused tropes ("mysterious past", "hidden power awakens", "an unlikely bond forms"). If an idea feels like it could appear in any generic story, discard it and find something more specific and surprising.');
        g.push('- ANTI-CLICHÉ: Do NOT default to the most obvious interpretation. Push past the first idea. If a setting/trait/relationship feels predictable, twist it or approach it from an unexpected angle.');

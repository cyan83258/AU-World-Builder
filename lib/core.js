...
(function (A) {
    ...
    A.DEFAULT_UPDATE_PROMPT = [
        ...
        '- Output ONLY changed sections, with complete replacement content inside their tags. Omit unchanged sections. If nothing changed, output [NO_CHANGES].',
        '- Maintain internal consistency: if one section changes, ensure related sections still make sense.',
        '- Treat dialogue claims, speculation, jokes and hypothetical plans as unconfirmed until established by events. Do not invent developments or expand prose just to fill the length guide.',
        '',
        '{{VOLUME_INSTRUCTION}}',
        '',
        '## Character Names',
    ...
    A.DEFAULT_SECTION_PROMPT = [
        ...
        '- Improve concrete choices, behavior and playable situations. Do not force secrets, twists or trauma into every section.',
    ...
    A.DEFAULT_BRAINSTORM_PROMPT = [
        ...
        'Based on the seed idea below, propose EXACTLY 3 distinct AU directions within the requested premise and preferences.',
        '',
        '## BRAINSTORM RULES',
        '- Differentiate ideas through roles, everyday mechanisms or reasons to interact; preserve requested genre and tone.',
        '- Each idea must have a clear narrative hook: a concrete situation that invites interaction, including peaceful or comedic situations when appropriate.',
        '- Ideas should leverage the characters\' unique traits in surprising ways.',
        '- Familiar settings are allowed. Make them specific through character behavior and practical circumstances, without requiring a twist.',
        '- Avoid three reskins of the same idea. Do not automatically add conspiracies, destined bonds or hidden powers.',
    ...
    A.DEFAULT_SKELETON_PROMPT = [
        ...
        '- A practical consequence connecting this section to the AU premise',
        '- One specific activity, choice or interaction that can actually occur in play; match the requested tone',
        '',
        '## CREATIVE DIRECTIVES',
        '{{GUIDELINES}}',
        '',
    ...
    A.fillTemplate = function (tmpl, vars) {
        // One pass: placeholders inside user/source text must remain literal.
        return tmpl.replace(/\{\{([A-Z_]+)\}\}/g, function (match, key) {
            return Object.prototype.hasOwnProperty.call(vars, key)
                ? String(vars[key] == null ? '' : vars[key]) : match;
        });
    };
